...
讀,它會為隔壁檔案宣告的 struct 報 `E707`、為 sibling 呼叫的 private function 報 `L102`、為一個坐在自己目錄旁邊而不
是裡面的 module 報 `E502`——三句在說正確程式碼的話。所以 driver 會**去找一個能走到這個 buffer 的 entry**:先看 buffer
...
session 檢查二十次:改之前在第三次就被 SIGKILL,改之後二十次都發出診斷,峰值 0.59 GB。省下來的不是 C 的大小——它只有
3.6 MB——而是把它組起來的形狀:`defs = defs + c_fn(…)` 在大約 1500 個步驟裡,每一步都把先前產出的全部再抄一份。
`make check-equal` 負責讓這兩條路誠實,而且它是逐位元組比對兩者的診斷的,因為一個比 build 找得少的檢查,就是一個對著
一份根本編不過的檔案顯示乾淨 buffer 的編輯器。
