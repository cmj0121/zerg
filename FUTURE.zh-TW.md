...
**狀態:開著,而且量過界限。** 由 0.2.0 指名 —— 那次發布把界限寫進規格,而不是留一個 deviation。

釋放一個遞迴值,每個節點吃掉一個 C stack frame,所以一條鏈能被釋放到多長,由釋放跑在哪條 stack 上決定 —— 在預設
8 MiB 主 stack 上大約六萬個節點(見[記憶體](docs/core/memory.zh-TW.md))。超過之後行程帶著名字死去,而且攔不住:
釋放跑在 scope 結束與 abort unwind 的路徑上,在那裡 raise 並不安全。

修法並不精巧 —— 把鏈走進一個明確的 worklist、一個節點一個節點釋放,而不是讓 C stack 當那個 worklist。讓它成為一次
修改而不是一次重寫的,是那個 drop 是**產生出來的**:每一個遞迴型別的拆解都出自 emitter 裡的同一處,所以形狀改一次
就好,不是每個型別改一次。

**如果要做**,要小心的是**順序**。遞迴的 drop 會先釋放一個節點的 payload、再釋放它的尾巴,而一個先推尾巴的 worklist
會把那個順序反過來 —— 一旦使用者能寫自己的 drop,那就是程式觀察得到的。

**門檻:一支真的需要比那個界限更深的鏈的程式**,而不是一份 benchmark。這套工具鏈建過的每一個遞迴結構都離那個界限
很遠,而一個真的要那麼深的結構,用一個依位置索引的 `list` 會更好 —— 那是語言已經有的形狀,而且一個迴圈就釋放完。
