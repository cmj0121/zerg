...
func TestRefEmit(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			// deref reads the payload; each Ref schedules its cleanup-stack release on
			// scope entry; del releases now and nulls the slot; the scope unwinds the
			// mark at its exit (running the survivor's guarded release).
			name:        "ref-construct-copy-deref-del",
			src:         "fn main() {\n r := Ref(7)\n s := r\n print deref(s)\n del s\n}",
			wantRuntime: true,
			wantContains: []string{
				"#include \"zergrt.h\"",
				"static void zg_ref_drop(void *slot)",
				"static void *zg_refnew_0(int64_t v)",
				"zrt_ref_alloc(sizeof(int64_t), NULL)",
				"= zrt_scope_mark();",            // scope entry mark
				"zg_r = zg_refnew_0(7)",          // construct: moved, not retained
				"zrt_defer(zg_ref_drop, &zg_r);", // schedule the survivor's release
				"zg_s = zrt_ref_copy(zg_r)",      // copy an lvalue: retain
				"zrt_defer(zg_ref_drop, &zg_s);",
				"zrt_ref_payload(zg_s)", // deref
				"zrt_release(zg_s);",    // del s: release now ...
				"zg_s = NULL;",          // ... and null the slot so exit skips it
				"zrt_unwind_to(",        // scope exit runs the survivor's release
			},
		},
		{
			// a construction bound directly is a move: no retain, one scheduled release.
			name:         "ref-move-no-retain",
			src:          "fn main() {\n r := Ref(1)\n}",
			wantRuntime:  true,
			wantContains: []string{"zg_r = zg_refnew_0(1)", "zrt_defer(zg_ref_drop, &zg_r);", "zrt_unwind_to("},
			wantAbsent:   []string{"zrt_ref_copy"},
		},
		{
			// a struct holding a Ref gets copy/drop helpers plus a drop-env thunk;
			...
			wantContains: []string{
				...
				"static void zg_dropenv_zg_Box(void *p) { zg_drop_zg_Box((zg_Box *)p); }",
				"zg_c = zg_copy_zg_Box(zg_b);", // copy an lvalue struct: deep copy
				"zrt_defer(zg_dropenv_zg_Box, &zg_b);",
				"zrt_defer(zg_dropenv_zg_Box, &zg_c);",
				"zrt_unwind_to(",
			},
		},
		{
		...
		},
		{
			...
			src:          "fn main() {\n r := Ref[int](9)\n print deref(r)\n}",
			wantRuntime:  true,
			wantContains: []string{"zg_refnew_0(9)", "zrt_defer(zg_ref_drop, &zg_r);", "zrt_unwind_to("},
		},
	}
