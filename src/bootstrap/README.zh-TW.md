...
- **`./` 錨定在 entry 檔的目錄,而不是寫下它的那個檔案。** `zerg` 把 `./` import 解析成相對於**匯入它的檔案**、
  把 `/` 解析成相對於 package 根;種子把兩者當成同一個根,所以 `sub/a.zg` 寫 `import "./b"` 而 `sub/b.zg` 就在
  旁邊時,`zerg` 解析得到而這裡不行。種子唯一的工作是建出那個編譯器,而它自己的 `./` import 全都寫在 entry 檔
  旁邊 —— 對它曾解析過的每一條路徑,兩個錨點都重合;要補這個洞,得把匯入者的目錄穿過一個沒有別的用處需要它的
  loader。
