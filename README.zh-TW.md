...
**`-o` 在每個階段的意思並不相同**，在信任它之前值得先知道：

| 階段            | `-o` 做的事                                                    |
| --------------- | -------------------------------------------------------------- |
| `--emit bin`    | 指定執行檔名稱——`-o app` 寫出 `app`                            |
| `--emit lib`    | 指定的是**字幹**，後面會再接上 `.o`——`-o out.o` 寫出 `out.o.o` |
| `--emit c`      | **被忽略**；C 一律走 stdout，請改用轉向——`--emit c f.zg > f.c` |
| `tokens`、`ast` | 同理被忽略——這兩個階段都寫到 stdout                            |

只有第一列符合 help 文字所描述的。另外兩者是 bug、不是設計：`--emit lib` 是把 `.o` 接在拿到的任何字串後面，而不
是只接在預設字幹後面；`--emit c` 則根本不去讀 `-o`。在那修好之前，給 `--emit lib` 的名字請不要帶副檔名。
