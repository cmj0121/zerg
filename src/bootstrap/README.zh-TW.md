...
- **持有東西的 tuple 不能被複製。** `t := (1, s)`（`s` 是 `str`），以及任何持有 `list` 或 `map` 的 tuple，
  ...
  作用域時把它的 `str` 還回去的原因。
- **宣告了兩次的型別名稱會被接受。** `struct`、`enum` 與 `spec` 共用同一個命名空間——所以同一個檔案裡的兩個
  `enum E`、兩個 `spec T`，以及 `struct A` 與 `spec A` 並存，都是一個名字對上兩份宣告。種子每一種都建得起來也
  跑得動。`zerg` 拒絕這樣的一對，並在兩者種類不同時把那兩個種類講出來。兩個檔案各自的私有型別，在兩個編譯器
  裡都是兩個型別；一個檔案宣告了自己的 `P`，又在型別位置寫了 `sink.P`，種子照建，`zerg` 則拒絕，因為型別位置
  會丟掉限定詞，兩者拼起來一樣。
...
- **沒有上界的 inclusive range 會被接受，而寫著它的那個 arm 永遠不會成立。** `GRAMMAR#range-arm` 規定
  ...
  不論它從哪裡來都拒絕這個形式（`E3136`）。種子把缺席的上界讀成 0，於是那個 arm 對每一個值都是假的，`match`
  就一聲不響地落到它的 catch-all。`reject-check.sh` 裡有一個案例帶著這個標記。
- **拿一個根本不是集合的東西當 `in` 的右邊會被接受，而由 cc 作答。** `in` 測的是成員資格，而一個集合是
  list 的元素、map 的鍵、range 的成員，或一個 error kind（[grammar](../../docs/surface/grammar.md)）；
  `3 in 5` 三者都不是，所以 `zerg` 拒絕它（`E3119`）。種子在這裡根本沒有讀這個運算子——它 emit 出
  `(3 ? 5)`，也就是少了第二個分支的 C 條件式，於是由 clang 對著沒有人寫過的生成程式碼作答。建置失敗不等於
  程式被拒絕，而這正是 `seed_refuses` 圍繞著寫的那個分別。`reject-check.sh` 裡有一個案例帶著這個標記。
...
- **泛型跨過 array、borrow 或函式型別不會被特化。** `zerg` 會把型別參數解出來、並代換過型別由之組成的每一種形狀;
  種子三種都不會:`fn first[T](xs: [T; 2])` 和 `fn put[T](mut &slot: T, v: T)` 會帶著它編不動的 C 走到 cc ——
  _field has incomplete type 'void'_ ——而 `fn apply[T](f: fn(T) -> T, …)` 它會用自己的話拒絕,
  _cannot use `fn(int) -> int` as `fn(T) -> T`_,那正是 `zerg` 在兩個走訪被綁在一起之前帶著的同一個缺陷。
