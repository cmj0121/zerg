...
An `F` or an `L` names what a **tool** does with a program that already builds. The compiler's
`E` codes name what stops a build, which is a different promise and a different page —
