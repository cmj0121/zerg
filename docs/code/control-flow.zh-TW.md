...
綁定位置**,同一個形狀就是同樣這三件事:[`GRAMMAR#struct-pat`](../../GRAMMAR) 只導出一條產生式、擺在兩個位置,
所以在一個 `P` 上寫 `Q{x} := p` 拿到的就是 arm 拿到的那個 _E4085_——**target** 是什麼、它和 pattern 差在哪,見
[Pattern](patterns.zh-TW.md)。**guard 條件**可用:一個 arm 可在 pattern 之後帶一個
