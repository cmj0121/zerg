...
`Object` spec（[Spec 與 Generics](specs.zh-TW.md)）——所以 `display` 與 `debug` 在每個值上都可用、不需 opt-in 任何
東西：
...
> **狀態。** 渲染一個**純量**或一個 **`str`**——透過純 `{x}` 洞、`print`，或 f-string——為 **[implemented]**。
...
- **`{x}`** 用 `display`；**轉換**可先換視圖——**`!r`** 用開發者 `debug`、**`!s`** 用 `display`、**`!a`** 用
  ASCII-escaped 的 debug。`f"{x!r}"` 把 `x` 以 `debug` 渲染。**`!s`** 為 **[implemented]**。
  > **[deviation]** `!r`（debug）與 `!a`（ascii）目前**別名到 `display`**——今日渲染得與 `!s` 一模一樣，而非
  > 開發者／ASCII-escaped 視圖。預期的獨立 `debug` 與 ASCII 渲染一如規格；別名會在結構化 `debug` 落地時消失。
...
- **`{x:spec}`** 把 spec 字串交給型別的 **`Format`** 協定——`f"{pi:.2f}"`、`f"{n:04d}"`、`f"{p:>10}"`。這是
  ...
  **純量或 `str`** 的 format spec 為 **[implemented]**。
