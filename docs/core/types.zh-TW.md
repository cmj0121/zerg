...
**宣告出來的型別，名字就是一個普通的 identifier**（[GRAMMAR#type-ident](../../GRAMMAR)）：名字不決定一個東西
是什麼。`Point` 之所以是型別，是因為有一個 `struct Point` 宣告了它；`point` 也一樣，`struct _Box` 也一樣。型別
用 PascalCase 是這套函式庫遵循的**慣例**，編譯器不強制。

`Point(1, 2)` 是建構、`point(1, 2)` 是呼叫，靠的是各自**被宣告成什麼**，不是怎麼拼——兩者共用一個命名空間，
所以同時宣告 `struct Fine` 與 `fn Fine` 是一個把兩者都指名的錯誤，一個名字在任一時刻最多只有一個意思在作用域裡。
呼叫一個沒有任何宣告的名字，是未定義的名字（`E4016`）；呼叫一個宣告過、但不是 `struct` 的型別，訊息會說出它是
哪一種（`E4064`）。

在 0.2.0 之前，第一個字母**就是**那條規則，而它的代價大於它買到的東西：`fn Fine` 是一個合法卻叫不動的宣告、一個
型別不能叫 `_Box`（只因為 `_` 根本沒有大小寫）、而 `point(1, 2)` 被拒絕時指名了一個程式從沒寫過的型別。
...
型別操作因此清楚分開：**建**一個新值（`T(x)`，這裡）、**測**一個值的身分（`x is T` → `bool`，見
[型別測試](specs.zh-TW.md)——錯誤種類比對它攜帶的 tag,其他每一個名字都是對運算元自己型別的編譯期常數;
**existential** 測試仍是 **[not yet]**,等的是 _E9048_）、以及**絕不**把一種型別的儲存重新詮釋成另一種。
