...
這種 impl 的 **associated fn** 經由不帶參數的型別取用——`Box.make(3)`——因為後綴的 `[…]` 是索引,從來不是型別
參數。impl 的參數如同泛型 `fn` 的參數,由呼叫解出,所以 `Box.make(3)` 就是 `Box[int]` 的 `make`。fn 在簽名與
本體中都沒寫到的參數沒有東西要決定——`fn one() -> int` 在每個 `T` 之下都是同一個本體;寫到了卻沒有任何引數能
決定的參數則被拒收(_E4009_)。
