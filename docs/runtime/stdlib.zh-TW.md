...
**`log.Logger()` 造出的與 `log.new()` 完全相同。** 每個欄位都帶預設值（module 私有欄位必須帶，`E4045`），
所以 `Logger()` 不管模組願不願意都存在，而在 `#[sealed]` 做出來之前（[Decorator](../core/decorators.zh-TW.md)），struct 字面值在其型別可見的地方
都能寫。從外面建構時會略去私有欄位，所以由它們的宣告決定：預設值**就是**環境變數，由 `log` 自己的私有 const 讀取，
而 `new` 就是一行 `return Logger()`。不存在會默默忽略環境變數的第二個建構子，`scripts/log-check.sh` 也讓兩者的答案一致。
...
十五個模組都能 import,這一個也是。它先後等過三種形式——generic struct、對它的應用做的 `impl`,以及 `Ref[T]`
——而三者都建好了。
