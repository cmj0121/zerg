...
> 的形狀,抵達讀者時是它自己的句子,而不是一次悄悄走歪的 parse。它是 **parser** 的代碼,指名的不是任何未建形式:
> `primary` 的每一個選項與每一個前綴,在它之上都有歸屬或自己的代碼,所以這個文法導得出的東西沒有一個會落到
> 這裡。
...
- **variant 或 binding** 由**語法**決定,永不由 scope 決定:variant **一律由它的 enum 指名**——`Color.Red`、
  ...
  的從來不是大小寫。因此 pattern 文法不需符號表即可剖析。從 0.2.0 起,這對**整個語言**都成立,而不只是 pattern:
  宣告出來的型別,名字就是一個普通的 identifier,一個名字的意思由宣告它的東西決定（見[型別](../core/types.zh-TW.md)）。
...
- **全域可變狀態。** _無可變全域_（group 10）的唯一例外，是**在 module 層級 `unsafe { … }` 分組內**的 `mut` 綁定——
  ...
  **atomics 是 stdlib、非文法**：`Atomic[T]` 提供 `load` / `store` / `swap` / `fetch_add` / `compare_swap` 與
  memory-ordering 參數。整片都是 **[not yet]**:隨附的 `atomic` 模組寫了 `impl Atomic[int]`,而對型別**引數**的
  `impl` 尚未建置,被拒絕的是那次 **import**——光是 `import "atomic"` 就會回報 _E9104 the module `atomic` ships and
  cannot be imported_,而不是讓拒收落在一個讀者沒寫過的檔案上（泛型 `struct` 已經建好;對它的應用做 `impl` 是
  `E9038`）。
