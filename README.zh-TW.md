...
`--emit lib f.zg -o out.o` 寫出的就是 `out.o`。之所以值得寫明，是因為它以前在每個階段的意思都不一樣：
`--emit c` 會直接丟掉這個旗標一律寫到 stdout——要求寫檔的建置什麼檔案都沒拿到，而且回傳 0。
各階段不同的只有沒給 `-o` 時的**預設值**：
...
種子較窄的子集是它自己的契約，記在 [`src/bootstrap/README.zh-TW.md`](src/bootstrap/README.zh-TW.md)，寫 Zerg 的讀者
永遠碰不到。
...
| 被賦值蓋掉的值是被丟下、不是被釋放——會漏                  | [值與記憶體](docs/core/memory.zh-TW.md) |
...
就是 `make help` 列出的那些 target；**`make test`** 跑整塊板，而 `make gates` 是用來擋下「只是掛在板上、
其實沒人跑」的關卡。
