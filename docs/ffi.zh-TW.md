...
| **import** | C → Zerg | 一個 **`extern`** 區塊，列出 Zerg 可呼叫的外部 C 符號               |

兩條邊共用**同一份**「哪些值可跨界」的定義（FFI-safe 型別）、**同一條**邊界記憶體所有權規則，以及**同一套**
錯誤與並行處理。`extern` 只屬於 import 方向，永不出現在 export 邊。
...
沒有。外部資源以 **opaque handle** 跨界：一個具名型別，在 `extern` 區塊中**無主體**地宣告，Zerg 能持有卻永遠
無法窺看。
...
一個 handle **可以**存進 binding 或欄位、被複製、傳給其他 `extern` 呼叫、以及被 `del`。它**不能**被解參考、
...
一個 `extern "C"` 區塊是外部符號進入 Zerg 的唯一門。每一項**原封不動**地命名一個 C 函式、opaque 型別或符號——
不套用任何 mangle，linker 名照字面採用。`extern` 簽章與任何邊界宣告一樣，會被檢查為 FFI-safe。
...
handle 並非 FFI-safe，不得出現在 `extern` 簽章或匯出面上；結果與完成仍只在 Zerg **內部**經由 channel 傳遞。
...
- **prelude 與 std** 不參與此機制：opaque handle 與 `extern` 是語言層級的、像 primitive 關鍵字般內建於工具鏈。
  std 仍可提供便利輔助（例如一個 `str` ⇄ C-string 橋接），但它們遵循上述同一套規則。
- **測試**把 `extern` 與匯出程式碼當作普通程式碼、依常規可見性規則處理；一個黑箱測試可以連結產生出的 header，
  完全如同一個 C 依賴者。
