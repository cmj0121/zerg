...
- 一個參數可宣告**預設值**——引數被省略時呼叫端所用的運算式。它在**每次被用到時於呼叫端求值**、絕不是在定義處
  ...
  預設值屬於**宣告**它的那個函式。它的名字照那個函式所在檔案的意思解讀；一個是前面參數的名字，讀的是**這次呼叫的
  引數**——寫出來的、被具名引數跳過的、或本身取預設值的都一樣——而呼叫端綁定的任何東西都看不到它：local、參數、
  迴圈或 `match` 的綁定、閉包的 capture 都不行。`fn g(a: int, b: int = a * 2)` 以 `g(3)` 呼叫時傳入 `b = 6`，
  不管呼叫寫在哪裡時 `a` 是什麼；呼叫端的 `beep := …` 也不會改變預設值裡的 `beep()` 呼叫的是誰。`spawn` 或
  `defer` 補上的預設值、以及 struct 欄位的預設值，都一樣。
  ...
  > parameter `x`_，並帶上位置。預設值是在**呼叫端**、從被呼叫者的**宣告**展開出來的，而
...
- 一個**具名引數**以參數名字傳入（`loud: true`）——這正是讓你能**跳過中間的預設參數**的關鍵。規則就是慣例那套：
  ...
  每一種把名字寫錯的方式都各自被具名拒絕：名字對應不到任何參數是 _E4106_、同一個參數被指名兩次是 _E4107_、
  具名之後又出現 positional 是 _E4108_、名字跳過了一個沒有預設可填的參數是 _E4109_。而在引數位置之外根本沒有
  參數可綁，那是 _E4105_——一次轉換、一個內建接收者的方法，兩者都有引數而都沒有可以指名的參數。
