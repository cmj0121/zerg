...
`Div{q, r} = divmod(x, y)`——每個葉子可以是任意 lvalue（`(a, obj.f) = …`）。兩個方向都是 **[not yet]**：編譯器
一次只綁一個名字，所以 tuple 形狀不論戴哪個運算子都被拒絕（_E238 a destructuring binding `(a, b) := …`_），而
struct 形狀則是它讀不懂的 pattern（_E221 a struct pattern `Div{…}`_）。
