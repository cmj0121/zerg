...
zerg doc --check strings      # 指名沒有註解的宣告；只要有就以 1 結束
```
...
會被指名——因為一個宣告根本沒進到文件裡的 module，會用「沒有東西可報」來回答「沒有東西缺席」——只要其中之一發生，這
一趟就以 **1** 結束。
...
會被兩個錯誤互相抵銷——寫一則註解、再加一個沒註解的 `pub`，數字沒有動，底下的集合卻動了兩次。
...
標點屬於哪一邊，所以中間帶逗號的句子就改斷在別處。

**作者自己的換行會變成一個空白，或什麼都沒有。** 在兩個詞之間，它代表的就是那個詞與詞之間的間隔；在兩個**全形**字元
之間，它變成什麼都沒有——否則一個沒有空白的語言就會憑空多出一個作者從來沒打過的字元，落在原始碼裡根本沒有間隔的句子
中間。這問的是**寬度**，而且刻意不是上面那個問題：全形逗號並不是一行**可以結束**的地方，因為這裡沒有任何東西知道一
個標點該靠哪一邊；但它仍然是一個不該有空白的地方。只有**一邊**是全形時，那個空白留著：`而 zerg doc 的排版` 正是本
樹自己的 zh-TW 文件寫出來的樣子，那個空白是作者自己放進去的。

`make doc-check` §7 就是把這件事做成 gate：它把一個註解用中文寫成的 module 產成文件，再量出來的每一行顯示欄寬——沒
有一行超過 80 欄，而最寬的那一行至少 70 欄，否則就是那一段根本沒被填過，前一個斷言等於什麼都沒量到。同一節還會讀那
些空白，因為那是任何欄寬都看不見的：沒有一行在兩個全形字元之間帶著空白，也沒有一行把漢字與西文詞之間的那個空白弄丟。
