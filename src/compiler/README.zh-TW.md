...
每個案例是一支 `.zg` 程式，旁邊放著它必須印出的 stdout、必須寫到 stderr 的內容，以及必須留下的
結束狀態。除了 [`scripts/corpus-skips.txt`](../../scripts/corpus-skips.txt) 點名的那幾個以外，每個
案例都是**閘門**：一個案例掉出這組就是 regression，會讓 target 失敗；而把一個名字從那個檔案裡刪掉，
**就是**那個名字所等的功能的閘門。

被擋下來的都是**指名**拒絕、不是誤譯，而且那一行會寫明是哪一個**錯誤碼**——`derive_ord` 等的是
_E9054_，`derive_enum` 等的是 _E9055_。

刪掉一個名字只是單向的閘門，而有一段時間那就是全部的機制:泛型 `struct` 建好之後，`gen_struct`
還留在清單上，而這一段散文說它建好了、清單卻仍然擋著那個案例。現在每一個被跳過的案例都會再送進
編譯器一次：**建得起來**就讓 gate 失敗，被**那一行沒有寫的錯誤碼**擋下來也一樣——後者是結束狀態
永遠看不到的那一半，因為任何一種失敗都會被讀成「還在等它的功能」，包括一個已經連 parse 都過不了
的案例。
