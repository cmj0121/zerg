...
而 `Ok(v)` 與 `Left(v)` 選一個 union 的邊——每一個都是一個帶著 tag 的複合字面量,哪裡都沒有配置。spec 定型的位置沒有地方放一個它不知道大小的值,所以
...
> 的 `list`,而不是一條鏈。
>
> **[implementation-defined]** 一條鏈**怎麼**被拆掉是這個實作的事、不是語言的事:上面那個遞迴 free 是一種做法,
> 顯式的 worklist 是另一種,而程式除了「第一種在多深的時候把 stack 用完」之外分辨不出它們。規範釘住的是那個界限
> 與那個診斷,那才是讀者要寫出待在界限內的結構所需要的。
...
> 以上表「釋放儲存」那一列，對擁有值而言是預期行為、尚非 bootstrap 現況——而在那之前，這個形式會被**具名拒絕**
> 而不是做一半：_E9108 NotImplemented: `del xs` on an owning list_。release 是在**宣告處**登記在 runtime 的
> cleanup stack 上、每個出口都退到一個 mark，所以沒辦法從中間抽掉一筆。**純量**的 `del` 不受影響：沒有儲存要
> 提早釋放，撤銷名字就是這個敘述對它的全部意義。
