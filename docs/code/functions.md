...
> `spawn`'s three callee forms — `spawn fn () { … }()` is _E4078_ — and `spawn work()` on the **named**
> closure above is _E9103_, with a place: both keywords lower to a C thunk whose body names a symbol, and a
> function value has none. Calling the literal where it is written, without `spawn`, is built. (It used to
> emit `zg_work()` into that thunk and die inside `cc`, which is the one outcome the standing rule
> forbids.) `spawn handle(x)` is the form
