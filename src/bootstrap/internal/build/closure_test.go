...
// TestCapturingClosureGated keeps the boundary: a closure that captures an enclosing
// local is reported (the environment representation is a later iteration), and
// capturing a `mut` variable gives the distinct always-illegal message.
func TestCapturingClosureGated(t *testing.T) {
	for _, tc := range []struct{ name, src, want string }{
		{
			"an immutable capture",
			"fn main() {\n\tbase := 100\n\tf := fn(n: int) -> int { return n + base }\n\tprint f(5)\n}\n",
			"captures a variable is not yet supported",
		},
		{
			"a mut capture",
