...
**它不會改寫無法 parse 的檔案。** 每一條規則都主張兩種寫法是同一個程式,而 parser 拒絕的檔案根本不是程式,無從
「相同」。這樣的檔案會以 `zerg build` 給出的同一個 finding 回報、以非零狀態結束,並原封不動地留下;`--check` 也
以同樣方式回答。這比[格式化工具](fmt.zh-TW.md)要求得更多:它只檢查括號是否平衡,好讓編譯器讀不懂的原始碼也能整理。
平衡檢查曾放過 `assert s == "x", f"bad {s}"`:對編譯器而言它是 `E2005`,而 `D104` 把 `,` 讀進了條件,寫出一個
tuple 條件,又被編譯器拒絕一次,而且是針對一行沒有人寫過的程式碼。
...
| `D105` | f-string 變回它所是的 `str` 串接      | 是     |

`D105` 寫出的正是 **parser** 為 f-string 建出來的東西——`f"a{x}b"` 就是 `"a" + str(x) + "b"`——所以一個 hole 的
尾巴會被寫成它所代表的那個呼叫:`{x!s}` 是 `x.display()`、`{x!r}` 是 `x.debug()`,而 `{x=}` 會把運算式自己的原始
文字放在前面。`:spec` 是 `x.format(spec)`,這個編譯器沒有建它(_E9107_),而 `!a` 根本沒有寫得出來的拼法——所以帶
著這兩者之一的字面量會**整個被放過**:一個 f-string 是一個 token,改一半就是一個不再說原本那句話的字面量。
...
### `D105`——f-string 變回它的串接

一個 f-string 就是一次 fold:先是開頭那段文字,然後每個 hole 一個部分,由左到右用 `+` 接起來。parser 建出來的正是
這個,所以這裡是把它寫出來、而不是用第二種方式再降一次——這也是 C 逐位元組相同的理由。
...
第一個部分**即使是空的也會被寫出來**,那就是開頭那個 `""` 的來歷:這個 fold 必須以一個 `str` 起頭,否則
`f"{a}{b}"` 加的就是兩個還不是字串的值。

一個 hole 的尾巴會被寫成它所代表的那個呼叫——`{x!s}` 是 `x.display()`、`{x!r}` 是 `x.debug()`——而運算式會被
**加上括號**,因為 `x + 1` 後面接一個 `.debug()` 是對 `1` 的呼叫。`=` 形式會把運算式自己的**原始文字**放在前面,
那是只有文字才有的事實。
