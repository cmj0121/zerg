...
> 而且因為把 decorator 帶到它的宣告上的,正是同一份名字清單,裸寫的形式哪裡也沒被帶到,於是根本沒碰到任何規則
> ——`#[derive]` 掛在一個 `fn` 上會編過,而同一個 `fn` 上的 `#[derive(Eq)]` 則會被拒絕、報
> _E487 `#[derive(Eq)]` applies to the `struct`, `enum` or `spec` that follows it, and what follows is `fn`_。
> 沒有任何東西被編錯,但一道指令被讀進來又被丟掉,而那正是封閉集合本來要排除的事。
