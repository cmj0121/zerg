...
**Sugar the grammar has and this table does not.** Nothing, as of #173 — the **named argument** `f(x: 1)`
was the last rewrite in that state and it is built, in a call and in a construction alike, together with the
default parameter that is the other half of its row. The whole list of forms still specified and not built,
with the gate that holds it, is
