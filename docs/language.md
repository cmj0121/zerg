...
| [Derive & Default Behavior](core/derive.md) | structural derivation vs spec default methods                     |
| [Decorators](core/decorators.md)            | the fixed, compiler-owned `#[…]` directive set                    |
...
| Chapter                                 | Covers                                              |
| --------------------------------------- | --------------------------------------------------- |
| [Syntax Sugar](surface/syntax-sugar.md) | every surface form and the core it desugars to      |
| [Grammar](surface/grammar.md)           | the formal surface grammar (companion to `GRAMMAR`) |
...
`is` type test (`x is T` on an error kind compares a runtime tag; on a boxed value it compares the witness
table the value carries; on any other name it is a compile-time constant from the operand's own type). See
...
**[Control Flow & Pattern Matching](code/control-flow.md)**.
...
**[Functions & Closures](code/functions.md)**.
...
- **`Ref(x)` / `deref(r)`** — construct and read the reference-counted box ([Values & Memory](core/memory.md)).
...
always-in-scope `print` keyword. See **[Formatting & Text](runtime/format.md)**.
...
tiers, and `is` dispatches on an erased `Err`. See **[Null-safety & Errors](code/errors.md)**.
