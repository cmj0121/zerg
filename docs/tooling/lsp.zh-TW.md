...
**函式**:給它一個 path 與編輯器手上的那些 buffer,拿回那個 path 所屬的整個程式,其中那些文字取代磁碟上的內容。
...
**而每一個開著的 buffer 都是那個程式的一部分**,不是只有被打字的那一個。交給 `serve` 的是整個文件庫,而走訪走到哪個
檔案,就用編輯器手上的那份文字頂替它;只有沒人開著的檔案才從磁碟讀。`a.zg` 裡還沒存檔的修改,就是開著的 `b.zg` 被拿
去檢查的那個程式的一部分;會從磁碟讀 `a.zg` 的 server,是拿一個沒有人擁有的程式在檢查 `b.zg`。

**一次修改會傳到 import 它的那些 buffer**,而回答它們全部的是同一次檢查。那一趟走訪本來就已經握有程式裡每個檔案的
finding,所以這個程式**含有**的每個開著的 buffer,都是把它說過的話按路徑切開之後發布出去的——除了本來就要跑的那次檢
查以外,不多花任何東西。在同一個程式的數個 buffer 的 session 裡,一次按鍵是一次檢查,外加每個 buffer 一次發布。

這個程式**沒有**含到的開著的 buffer,會被問另一個問題——**它自己的**程式有沒有含到剛剛改動的那個檔案——而這個問題是
問 loader 走出來的 import 圖,不是問名字怎麼拼;只有答案是「有」的時候才檢查它。這不是在補上面那個搜尋的某個角落。
那個搜尋回答的是**一個**程式,也就是第一個走得到這個 buffer 的候選 entry;而「哪些開著的 buffer 因為這個檔案改了而
過時」有幾個含有它的程式就有幾個答案:兩個 entry import 同一個 module 是一般的形狀,而 `lib.zg` 和坐在它旁邊的
`main.zg`——`lib.zg` 自己的程式就只有 `lib.zg`,因為搜尋從不看 buffer 自己的目錄——是最小的那個。回答了這個問題的那
個程式,就是被拿去檢查的那一個,所以一個 dependant 付的是一次載入加一次檢查,而不是兩次載入。

**這個問題是每個程式問一次,不是每個 buffer 問一次。** 一次載入是 import 走訪加上把程式裡每個檔案剖析一遍,而同一個
程式的那些 buffer 共用同一個答案:它們就在這個程式裡,所以這也是它們的程式。若是每個 buffer 各問一次,另一個程式的四
個 buffer 就會讓每一次按鍵都多付四次完整載入——`make lsp` 把它量成一個比值,對照只開著一個這種 buffer 的 session。

**一個 buffer 在一輪裡最多發布一次,而由這一輪的主體決定是哪一次檢查替它說話。** 一個檔案可以同時屬於兩個程式——被
`one.zg` 和 `two.zg` 同時 import 的 `shared/mod.zg` 就是——而一輪裡若兩個程式都被檢查,這個檔案就會被發布兩次,編輯器
顯示的是最後落地的那一份。主體的程式含有的 buffer,由那個程式回答;其他的,由它自己的程式回答。

**關閉**一個 buffer 也是這樣發布的,因為被關掉的檔案又回到磁碟那一份——那是一次修改造成的同一種過時,只是從另一個方
向過來。

**一個 lex 不過的 buffer 是一則 finding,不是一個洞。** 不能斷詞的原始碼就不是程式,所以載入會拒絕,而不是少了那個檔
案繼續往下走——剖析不過的檔案本來就是這樣。那則詞法 finding 發布在**那個** buffer 裡,而程式含有它的那些 buffer 則原
封不動:拿一個少了檔案的程式去檢查,開著的 `main.zg` 會在 `lib.zg` 的引號還沒收起來的那段時間裡,被告知
`` E3084 module `lib` has no `greet` ``——一句在說正確程式碼的話。它們的診斷只是舊了一瞬,而不是錯的;引號收起來後的下一
次按鍵就會把它們補上。

**有兩件事編輯器看到的比命令列少。** 關於一個**沒有人開著**的檔案的 finding 不會發布到任何地方——協定把診斷寄給一份
文件,而這裡沒有那份文件;`zerg build` 會印它,編輯器不會。另外,編譯器在**自己寫出來的**樹裡提出的 finding——
`#[derive]` 展開出來的樹是在 `<derive:FILE>` 上走訪的——會發布在 FILE 的 buffer 裡,放在寫出那棵樹的 **decorator** 上,
而不是它自己那個行與欄,因為那個位置指的是一段沒有人看得到的文字。同一個檔案的每個展開都共用那個路徑,所以 finding
會指名它是為哪個宣告展開的,而那個宣告的 decorator 就是位置。在展開裡中止的 abort 也這樣放:放在 FILE 的 buffer,
而不是觸發那次檢查的 buffer。沒有指名任何宣告的 finding——例如 `#[obj]` 寫出的包裝函式
裡的——會放在檔案開頭。

所以一輪裡的每一次檢查,都是對一個含有那個被改動的 buffer 的程式的檢查,而這正是讓[名稱索引](#一個名字從同一份索引回答)
仍然答得出它的原因。
...
**abort 單獨發佈,與 `zerg lint` 印出的一樣。** raise 它的走訪從未走到檢查器的規則,所以它旁邊沒有任何 lint finding
([兩個 exit code](lint.zh-TW.md#嚴重度以及它決定的兩個-exit-code))。走訪收集後繼續走下去的錯誤,旁邊仍保留
lint finding,在編輯器裡與在命令列上都一樣。
...
什麼都不改——而找出它真的改了的那兩種情況(一條被拆行的 `+` 鏈,與一行讀起來像以運算子結尾之程式碼的範例註解:
當時是單獨的 `# >>>` 提示符,現在是 ` ```zerg ` 圍欄裡的一行),就是這條規則被塑造出來的過程。
...
宣告;顯示出來的則是 **`zerg doc` 為它印的那份文件**——寫在它上面那則註解裡給讀者的文字,也就是它的 `##` 行,以及編
譯器拼出來的簽名。`#` 那一行是維護者的,不會出現在任何 hover 裡,就像它不會出現在預設的頁面上。
...
——同一個編譯器型別印表器印出的簽名,以及沒寫任何 `##` 行時同一個 `(undocumented)`。一次抽取,兩個讀者。
...
它的方式載入,並以每個開著的 buffer 頂替磁碟上的內容。因此 hover 回答的是正在被打出來的那份文字,而一個 raise 掉的
