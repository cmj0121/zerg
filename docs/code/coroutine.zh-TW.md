...
> 這則回報只有訊息，沒有別的：它不會指出自己來自**哪個 coroutine**、不印 **backtrace**、也不讀任何決定要不要
> 印的 **compiler flag**——所以一個有很多 coroutine 的程式拿得到原因，卻沒有對應的 `spawn` 位置可以掛上去。那是
> 這則回報的限度，而不是一個等著被建的形式：這裡沒有任何程式寫得出來、然後被拒絕的東西。
