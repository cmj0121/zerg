...
[Null-safety & Errors](errors.md)). This phase, **that is the only implemented use** — `is` works on the
...
fixed-array size (`[byte; Buffer.SIZE]`, see [Collections](collections.md)). Visibility is the ordinary
...
`debug` — belong to [Format](format.md); their **structural auto-derivation** is **[not yet]**. Everything
