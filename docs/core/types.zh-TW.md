...
- **同一條規則適用於每一個數值目標,而 `byte` 是最值得講清楚的那個。** untyped 字面量會採用它——
  `b: byte = 5` 和 `xs: list[byte] = [1, 2]` 就是把 byte 寫成 byte——而**一個 `byte` 值永遠不會加寬成
  `int`**:`b'J'` 是 `byte`,所以 `n: int = b'J'`、把 `b'a'` 傳進 `int` 參數、從 `-> int` 回 `b'a'`、
  以及把 `[b'a']` 放進 `list[int]`,全都是錯誤,而 `int(b)` 才是它變成數字的方式。`byte` 裝得進 `int`
  不是隱式轉換的理由,那正是這個轉換很便宜的理由。**採用講的是還沒有型別的字面量,加寬講的是已經有
  型別的值**,而只有前者會自己發生。
