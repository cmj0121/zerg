...
- **Integer overflow and division by zero raise** (`OverflowError`, `DivideByZeroError`) — an
  **abort**, not a value (see [Null-safety & Errors](errors.md)); `int`/`uint`/`byte`/`rune` never wrap (opt into roll-over
  with `+%`/`-%`/`*%`, below).
...
destructured — so multiple return needs no separate mechanism ([Pattern matching](control-flow.md)).
...
explicit wrapping (see [Null-safety & Errors](errors.md)) — still a build of the target value, never a reinterpret.
