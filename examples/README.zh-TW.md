...
這個目錄裡的每一支程式，附閱讀順序。每一支都由 **`make examples` 建起來並執行**、由 **`make lint`
檢查**、也由 **`make fmt-self` 釘在 `zerg fmt` 的形狀上**，所以這裡沒有「以前能動」的片段，也沒有一支
會被我們叫讀者去跑的工具要求修改——那兩支**應該被拒收**的，也被釘在它們必須被拒收的那句話上。
