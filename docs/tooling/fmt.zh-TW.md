...
| `E6xx` | 剖析 | 剖析器再一次:`E2xx` 滿了,而一段就是一百個號碼 |

`E5xx` 是唯一不落在那個順序裡的一段。一次建置在詞法分析被 import 指到的檔案之前就先解析 import,
並在一切都產出之後才去找 `fn main`,所以驅動器自己的發現是把另外四段夾在中間,而不是坐在其中兩段之間。

`E6xx` 根本不是一個階段——它是 **`E2xx` 的延續**。一段就是一百個號碼,而剖析器用掉了其中九十八個;
剖析器的拒絕被移到同一條回報通道上時新增的那些規則,在 `E2xx` 裡已經沒有地方可去。在一段新的號碼裡
繼續下去,是唯一還能保住這套編號那兩個性質的作法:號碼永不重用,而遇到它的讀者仍看得出是哪個階段在說話。

**一段用滿時,為那個階段開一段延續的號碼,並把舊的那段關掉。** 一個階段有兩段開著的號碼,就是有兩個
地方可以配號,而那正是一週之內撞三次的那個條件。「關掉」就是下面退場表裡 `E299` 在做的事:它從未被
發出過,而退場是把一個號碼在沒有被花掉的情況下退出流通的方式,好讓 `E2xx` 讀起來是**滿的**,而不是
還剩一格給人拿。`E4xx` 離同樣的位置只差一個號碼,而產出階段的延續和這一次是同樣的、寫在紙面上的
成本——這裡多一列,別無其他。
...
的那一段——延續的意思就是這個。因此新增一段延續的號碼就是在這裡多一列,建議會跟著那一列走。
...
| `E601` | `…` 需要一個名字,而 `…` 不是                                                                          |
| `E602` | `<-` 前綴是一個 channel 方向:只有 `<-chan[T]` 是型別                                                  |
| `E603` | `impl` 裡宣告前面的 `mut` 標記的是 `mut fn` 方法,而這個不是 `fn`                                      |
| `E604` | `is` 的右邊要一個型別名字                                                                             |
| `E605` | `…` 是一個 statement,而這裡要的是 expression — **[not yet]**                                          |
| `E606` | `…` 不是這個編譯器讀得懂的 expression — **[not yet]**                                                 |
| `E607` | match arm 的 body 是一個 expression,而這個是 statement — **[not yet]**                                |
| `E608` | f-string 的字面文字格式不正確                                                                         |
...
| `E299` | 從未發出：`E2xx` 在 `E298` 關閉，而剖析器的號碼在 `E6xx` 繼續    |
