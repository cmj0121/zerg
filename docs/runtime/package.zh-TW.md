...
>
> **一個公開名字不是全程式唯一的。** 兩個 module 各自宣告 `pub fn helper` 是可以的,規格說它們是兩個不同的
> 名字——這一頁刻意拒絕全域註冊表,所以根本沒有一個「讓公開名字唯一」的單位。過去會撞的只有 C 符號,而修法是
> 停止扣住一個早就存在的 tag:兩個 module 各自宣告的**私有**名字一直是靠 module tag 分開的——它在「程式的
> module 名字排序後」的位置。用排序而不是首見順序,是因為那個名字每次跑都必須一樣。公開的現在拿到同一個 tag
> (`examples/1g/pubname`,#92)。

**做選擇的是帶限定的呼叫。** `a.helper()` 把 module 說了出來,所以讀可見性的那一列與發出的符號都是
**命名空間**的。過去每個呼叫點問的都是**呼叫端**的 module——那對私有名字是對的,裸呼叫一定指呼叫端自己那一個
——而那正是公開撞名分不開的原因。

同一個目錄下的兩份宣告是另一條規則,而且仍然被拒絕:不論分成幾個檔案,它們是**同一個** module,沒有 tag 能把
它們分開——_E4077 `a.zg` and `b.zg` both define `work`_。那個問題會**先**問。

> 今天沒有任何東西能從 C 指名一個 `pub` 符號,這正是這個 tag 現在可以自由套用的原因:[FFI](ffi.zh-TW.md) 把
> 整條 export 邊標為未建,`--emit lib` 只寫 object、不產生 header,也沒有任何東西回報 header 會帶哪些宣告。
> 等那條邊落地時,它會一併帶來那一頁已經保留的 **link-name 覆寫**,而那才是外部呼叫端需要的退出機制。
...
> 以上全部都到不了，而這不是第二個 `[not yet]`：帶著這份缺席的是「四層」底下那個標記。沒有 package，就沒有
> 版本、package 之間沒有圖，也沒有東西可選。
...
私有型別不在那個空間裡。兩個檔案各自不加 `pub` 宣告同名的 struct、enum 或 `type`，宣告的是兩個型別，每個檔案的
建構、欄位讀取、pattern 與 method 都指向自己那一個。會被拒絕的是：一個檔案宣告了自己的 `P`，又在型別位置寫了
另一個 module 的 `sink.P`——型別位置會丟掉限定詞，兩者會拼成同一個名字，那就是 _E3158_。
...
> 被比較的邊界是 **module** 邊界,因為上面那個 package 層並不存在、無法成為另一邊。所以就編譯器
...
**四個前綴、四個根,而且沒有搜尋順序。** 讀一個 import 就知道它從哪來,而磁碟上的任何東西都改變不了那件事 ——
一個檔案出現或搬走,只能改變它**解不解析得到**。

| 寫法                      | 在哪裡找                            |
| ------------------------- | ----------------------------------- |
| `import "io"`             | 標準函式庫                          |
| `import "/http"`          | 本 package 的根 —— entry 檔所在之處 |
| `import "./http"`         | 寫下這個 import 的檔案旁邊          |
| `import "github.com/a/b"` | 遠端 package —— 形狀保留,具名拒收   |

兩個本地前綴在一個檔案不在根上時就分開了,而那正是它們的用途:`/` 讓深處的檔案指名上層的模組,不必寫
`./../..`;`./` 指名鄰居的方式,能在整個資料夾被搬走之後仍然成立。import path 從來不是檔案系統路徑 —— `/x` 是
package 根底下的 `x`,不是 `/usr/x`。
...
> **[not yet]** 上述之外還有三件事：把 doc 範例當測試跑、benchmark，以及**同時跑兩個測試**
